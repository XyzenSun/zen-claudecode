...
## Step 0 :前置检查与Git仓库初始化

1. 检查本地当前工作目录是否存在Git仓库，如果没有 使用 git init 初始化仓库，并询问用户是否有远程仓库，如果用户提供远程仓库，则添加此远程仓库（设为origin），如果用户不提供，则忽略
2. 调用 Bash `date +%Y-%m-%d` 获取当前日期，记作 `today`（用于后续文件的模板替换）。
3. 用 Bash 检查当前工作目录下是否已存在 `./zen-learn/`：
